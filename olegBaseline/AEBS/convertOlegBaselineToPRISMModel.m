function [] = convertOlegBaselineToPRISMModel(lattice, maxN, fileName,deltad,deltav,amax,trimmed)
...
% N=5, p(det)=0.9
% fprintf(fid, '    [] currN=0&currK=0&did>1&vid>1 -> 0.00001:(currN''=1)&(currK''=0) + 0.00045:(currN''=1)&(currK''=1) + 0.0081:(currN''=1)&(currK''=2) + 0.0729:(currN''=1)&(currK''=3) + 0.32805:(currN''=1)&(currK''=4) + 0.59049:(currN''=1)&(currK''=5);\n\n');
% fprintf(fid, '\n \n');
        ...
        for k = 1:length(currMapKeys)
            ...
            for l = 1:length(val)
                ...
                for m = 1:size(nextPointsTrans,1)
                    nextPointdid = nextPointsTrans(m,1);
                    nextPointvid = nextPointsTrans(m,2);
                    if((~(nextPointdid==1)) && ~(nextPointdid==2 && nextPointvid>1))
%                     if(nextPointdid>1)
                        allDidNextAre1=0;
                        break
                    end
...
%                     fprintf(fid, '    [] vid=%i&did=%i&currN=%i&currK=%i -> (vid''=%i)&(did''=%i)&(currN''=0)&(currK''=0);\n', currvid, currdid, currN, currK, nextPointvid, round(nextPointdid));
                ...
                for l = 1:length(val)
                    ...
                    for m = 1:size(nextPointsTrans,1)
                        ...
                        if(nextPointdid==2 && nextPointvid>=2)
