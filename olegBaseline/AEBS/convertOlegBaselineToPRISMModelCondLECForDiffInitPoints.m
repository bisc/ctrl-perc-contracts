function [] = convertOlegBaselineToPRISMModelCondLECForDiffInitPoints(lattice, maxN, fileName,deltad,deltav,amax,trimmed,safetyThresh,windowLen)
...
% N=1, p(det)=0.9
% fprintf(fid, '    [] currN=0&currK=0&did>1&vid>1 -> 0.1:(currN''=1)&(currK''=0) + 0.9:(currN''=1)&(currK''=1);\n\n');
fprintf(fid, '\n \n');


if(windowLen==3)
    fprintf(fid, '    [] currN=1&(s+s2+s3)<=1 -> (currN''=2)&(currK''=1);\n');
    fprintf(fid, '    [] currN=1&(s+s2+s3)>1 -> (currN''=2)&(currK''=0);\n');
end
...
safetyThreshInd = floor(safetyThresh/deltad)+1;
...
%                          fprintf(fid, '    [] vid=%i&did=%i&currN=%i&currK=%i -> (vid''=%i)&(did''=%i)&(currN''=0)&(currK''=0);\n', currvid, currdid, currN, currK, nextPointvid, round(nextPointdid));
                           %% Stateless LEC precomputed K and N transitions
                            fprintf(fid, '    [] vid=%i&did=%i&currN=%i&currK=%i -> (vid''=%i)&(did''=%i)&(currN''=0)&(currK''=0);\n', currvid, currdid, 2, currK, nextPointvid, round(nextPointdid));
                        end
                    end
                end
%             end
