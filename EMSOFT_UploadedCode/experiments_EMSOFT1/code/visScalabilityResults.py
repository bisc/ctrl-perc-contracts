...
if(showOlegBaseline):
    ax.scatter3D(YSmall, XSmall, crashProbsArrayOlegBaseline, color='green')
if(showOlegBaselineWithRounding):
    ax.scatter3D(Y, X, crashProbsArrayOlegBaselineWithRounding, color='red')
if(showExactBaseline):
    ax.scatter3D(Y, X, crashProbsArrayExactBaseline, color='black')
if(showBaselineWithRounding):
    ax.scatter3D(Y, X, crashProbsArrayBaselineWithRounding, color='purple')
if(showLattice):
    ax.scatter3D(Y, X, crashProbsArrayLattice, color='blue')
if(showRamneetKN):
    ax.scatter3D(Y, X, crashProbsArrayRamneetKandN, color='brown')
...
if(showOlegBaseline):
    ax2.scatter3D(YSmall, XSmall, np.log10(runTimesArrayOlegBaseline), color='green') # was purple, now green
if(showOlegBaselineWithRounding):
    ax2.scatter3D(Y, X, np.log10(runTimesArrayOlegBaselineWithRounding), color='red') # done
if(showExactBaseline):
    ax2.scatter3D(Y, X, np.log10(runTimesArrayExactBaseline), color='black') # done
if(showBaselineWithRounding):
    ax2.scatter3D(Y, X, np.log10(runTimesArrayBaselineWithRounding), color='purple') # done
if(showLattice):
    ax2.scatter3D(Y, X, np.log10(runTimesArrayLattice), color='blue') # done
if(showRamneetKN):
    ax2.scatter3D(Y, X, np.log10(runTimesArrayRamneetKandN), color='brown') # done
