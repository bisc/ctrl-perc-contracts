...
for distDisc in distDiscs:
    ...
    modelFile = open("windowed/diffDistDiscsNew/lecBaselineAllVoteFilterH" + str(H) + "L" + str(L) + "DistDisc" + str(distDisc) + ".prism",'w')
    ...
    if(H==2):
        ...
        modelFile.write("    [] seqflag=2&(s+s2<=0)&contRegion=0&carSpeed>0&carPos>0 -> (carPos'=max(0,floor(carPos-" + str(speedDisc/distDisc) + "*ceil(carSpeed/freq))))&(seqflag'=0);\n")
        modelFile.write("    [] seqflag=2&(s+s2<=0)&contRegion=1&carSpeed>0&carPos>0 -> (carPos'=max(0,floor(carPos-" + str(speedDisc/distDisc) + "*ceil(carSpeed/freq))))&(carSpeed'=max(0,carSpeed-floor(B1/freq)))&(seqflag'=0);\n")
        modelFile.write("    [] seqflag=2&(s+s2<=0)&contRegion=2&carSpeed>0&carPos>0 -> (carPos'=max(0,floor(carPos-" + str(speedDisc/distDisc) + "*ceil(carSpeed/freq))))&(carSpeed'=max(0,carSpeed-floor(B2/freq)))&(seqflag'=0);\n")
        modelFile.write("    [] seqflag=2&(s+s2>0)&contRegion=0&carSpeed>0&carPos>0 -> (carPos'=max(0,floor(carPos-" + str(speedDisc/distDisc) + "*ceil(carSpeed/freq))))&(seqflag'=0);\n")
        modelFile.write("    [] seqflag=2&(s+s2>0)&contRegion=1&carSpeed>0&carPos>0 -> (carPos'=max(0,floor(carPos-" + str(speedDisc/distDisc) + "*ceil(carSpeed/freq))))&(seqflag'=0);\n")
        modelFile.write("    [] seqflag=2&(s+s2>0)&contRegion=2&carSpeed>0&carPos>0 -> (carPos'=max(0,floor(carPos-" + str(speedDisc/distDisc) + "*ceil(carSpeed/freq))))&(seqflag'=0);\n\n")
    

    if(H==3):
        modelFile.write("    // compute carPos, carSpeed\n")
        modelFile.write("    [] seqflag=2&(s+s2+s3<=0)&contRegion=0&carSpeed>0&carPos>0 -> (carPos'=max(0,floor(carPos-" + str(speedDisc/distDisc) + "*ceil(carSpeed/freq))))&(seqflag'=0);\n")
        modelFile.write("    [] seqflag=2&(s+s2+s3<=0)&contRegion=1&carSpeed>0&carPos>0 -> (carPos'=max(0,floor(carPos-" + str(speedDisc/distDisc) + "*ceil(carSpeed/freq))))&(carSpeed'=max(0,carSpeed-floor(B1/freq)))&(seqflag'=0);\n")
        modelFile.write("    [] seqflag=2&(s+s2+s3<=0)&contRegion=2&carSpeed>0&carPos>0 -> (carPos'=max(0,floor(carPos-" + str(speedDisc/distDisc) + "*ceil(carSpeed/freq))))&(carSpeed'=max(0,carSpeed-floor(B2/freq)))&(seqflag'=0);\n")
        modelFile.write("    [] seqflag=2&(s+s2+s3>0)&contRegion=0&carSpeed>0&carPos>0 -> (carPos'=max(0,floor(carPos-" + str(speedDisc/distDisc) + "*ceil(carSpeed/freq))))&(seqflag'=0);\n")
        modelFile.write("    [] seqflag=2&(s+s2+s3>0)&contRegion=1&carSpeed>0&carPos>0 -> (carPos'=max(0,floor(carPos-" + str(speedDisc/distDisc) + "*ceil(carSpeed/freq))))&(seqflag'=0);\n")
        modelFile.write("    [] seqflag=2&(s+s2+s3>0)&contRegion=2&carSpeed>0&carPos>0 -> (carPos'=max(0,floor(carPos-" + str(speedDisc/distDisc) + "*ceil(carSpeed/freq))))&(seqflag'=0);\n\n")
    
    if(H==4):
        modelFile.write("    // compute carPos, carSpeed\n")
        modelFile.write("    [] seqflag=2&(s+s2+s3+s4<=0)&contRegion=0&carSpeed>0&carPos>0 -> (carPos'=max(0,floor(carPos-" + str(speedDisc/distDisc) + "*ceil(carSpeed/freq))))&(seqflag'=0);\n")
        modelFile.write("    [] seqflag=2&(s+s2+s3+s4<=0)&contRegion=1&carSpeed>0&carPos>0 -> (carPos'=max(0,floor(carPos-" + str(speedDisc/distDisc) + "*ceil(carSpeed/freq))))&(carSpeed'=max(0,carSpeed-floor(B1/freq)))&(seqflag'=0);\n")
        modelFile.write("    [] seqflag=2&(s+s2+s3+s4<=0)&contRegion=2&carSpeed>0&carPos>0 -> (carPos'=max(0,floor(carPos-" + str(speedDisc/distDisc) + "*ceil(carSpeed/freq))))&(carSpeed'=max(0,carSpeed-floor(B2/freq)))&(seqflag'=0);\n")
        modelFile.write("    [] seqflag=2&(s+s2+s3+s4>0)&contRegion=0&carSpeed>0&carPos>0 -> (carPos'=max(0,floor(carPos-" + str(speedDisc/distDisc) + "*ceil(carSpeed/freq))))&(seqflag'=0);\n")
        modelFile.write("    [] seqflag=2&(s+s2+s3+s4>0)&contRegion=1&carSpeed>0&carPos>0 -> (carPos'=max(0,floor(carPos-" + str(speedDisc/distDisc) + "*ceil(carSpeed/freq))))&(seqflag'=0);\n")
        modelFile.write("    [] seqflag=2&(s+s2+s3+s4>0)&contRegion=2&carSpeed>0&carPos>0 -> (carPos'=max(0,floor(carPos-" + str(speedDisc/distDisc) + "*ceil(carSpeed/freq))))&(seqflag'=0);\n\n")

    if(H==5):
        modelFile.write("    // compute carPos, carSpeed\n")
        modelFile.write("    [] seqflag=2&(s+s2+s3+s4+s5<=0)&contRegion=0&carSpeed>0&carPos>0 -> (carPos'=max(0,floor(carPos-" + str(speedDisc/distDisc) + "*ceil(carSpeed/freq))))&(seqflag'=0);\n")
        modelFile.write("    [] seqflag=2&(s+s2+s3+s4+s5<=0)&contRegion=1&carSpeed>0&carPos>0 -> (carPos'=max(0,floor(carPos-" + str(speedDisc/distDisc) + "*ceil(carSpeed/freq))))&(carSpeed'=max(0,carSpeed-floor(B1/freq)))&(seqflag'=0);\n")
        modelFile.write("    [] seqflag=2&(s+s2+s3+s4+s5<=0)&contRegion=2&carSpeed>0&carPos>0 -> (carPos'=max(0,floor(carPos-" + str(speedDisc/distDisc) + "*ceil(carSpeed/freq))))&(carSpeed'=max(0,carSpeed-floor(B2/freq)))&(seqflag'=0);\n")
        modelFile.write("    [] seqflag=2&(s+s2+s3+s4+s5>0)&contRegion=0&carSpeed>0&carPos>0 -> (carPos'=max(0,floor(carPos-" + str(speedDisc/distDisc) + "*ceil(carSpeed/freq))))&(seqflag'=0);\n")
        modelFile.write("    [] seqflag=2&(s+s2+s3+s4+s5>0)&contRegion=1&carSpeed>0&carPos>0 -> (carPos'=max(0,floor(carPos-" + str(speedDisc/distDisc) + "*ceil(carSpeed/freq))))&(seqflag'=0);\n")
        modelFile.write("    [] seqflag=2&(s+s2+s3+s4+s5>0)&contRegion=2&carSpeed>0&carPos>0 -> (carPos'=max(0,floor(carPos-" + str(speedDisc/distDisc) + "*ceil(carSpeed/freq))))&(seqflag'=0);\n\n")

    if(H==7):
        modelFile.write("    // compute carPos, carSpeed\n")
        modelFile.write("    [] seqflag=2&(s+s2+s3+s4+s5+s6+s7<=0)&contRegion=0&carSpeed>0&carPos>0 -> (carPos'=max(0,floor(carPos-" + str(speedDisc/distDisc) + "*ceil(carSpeed/freq))))&(seqflag'=0);\n")
        modelFile.write("    [] seqflag=2&(s+s2+s3+s4+s5+s6+s7<=0)&contRegion=1&carSpeed>0&carPos>0 -> (carPos'=max(0,floor(carPos-" + str(speedDisc/distDisc) + "*ceil(carSpeed/freq))))&(carSpeed'=max(0,carSpeed-floor(B1/freq)))&(seqflag'=0);\n")
        modelFile.write("    [] seqflag=2&(s+s2+s3+s4+s5+s6+s7<=0)&contRegion=2&carSpeed>0&carPos>0 -> (carPos'=max(0,floor(carPos-" + str(speedDisc/distDisc) + "*ceil(carSpeed/freq))))&(carSpeed'=max(0,carSpeed-floor(B2/freq)))&(seqflag'=0);\n")
        modelFile.write("    [] seqflag=2&(s+s2+s3+s4+s5+s6+s7>0)&contRegion=0&carSpeed>0&carPos>0 -> (carPos'=max(0,floor(carPos-" + str(speedDisc/distDisc) + "*ceil(carSpeed/freq))))&(seqflag'=0);\n")
        modelFile.write("    [] seqflag=2&(s+s2+s3+s4+s5+s6+s7>0)&contRegion=1&carSpeed>0&carPos>0 -> (carPos'=max(0,floor(carPos-" + str(speedDisc/distDisc) + "*ceil(carSpeed/freq))))&(seqflag'=0);\n")
        modelFile.write("    [] seqflag=2&(s+s2+s3+s4+s5+s6+s7>0)&contRegion=2&carSpeed>0&carPos>0 -> (carPos'=max(0,floor(carPos-" + str(speedDisc/distDisc) + "*ceil(carSpeed/freq))))&(seqflag'=0);\n\n")
