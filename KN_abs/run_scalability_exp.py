...
# python run_scalability_exp.py -start_exp 7 -end_exp 7 -m 3 -init_v 200 -init_pos 1600  
#######################################################################################################
...
def run(args):
    ...
    folder_name = 'ramneet_automated_exp_results/scalability/m{}'.format(m)
    if not os.path.exists(folder_name):
        os.makedirs(folder_name)

    for exp in range(start_exp,end_exp+1):
        for init_position in range(1200,1450, 50):
            for init_vel in range(100,300,40):
                init_v = float(init_vel)
                init_pos = float(init_position)
                print("init_vel: {}, init_pos: {}".format(init_v, init_pos))
                prism_in = '{}/pos{}_v{}.csv'.format(folder_name,init_pos,init_v)
                prism_out = '{}/pos{}_v{}.prism'.format(folder_name,init_pos,init_v)
                ...
                #prop = prop[:-1]
                prop += ')]'
                with open('{}/pos{}_v{}.props'.format(folder_name,init_pos,init_v), 'w') as prop_file:
