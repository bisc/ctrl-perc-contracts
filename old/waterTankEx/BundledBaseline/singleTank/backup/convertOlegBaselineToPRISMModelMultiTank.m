...
for i=1:numTanks
    fprintf(fid, '    wlid%i : [1..wlidMax] init wlidInit%i;\n',i,i);
    fprintf(fid, '    wlidPer%i : [1..wlidMax] init wlidInit%i;\n',i,i);
        ...
        for k = 1:length(currMapKeys)
            ...
            for l = 1:length(val)
                ...
                for m = 1:size(nextPointsTrans,1)
                    ...
                    if(nextPointwlid<=1 || currwlid<=1)
                        fprintf(fid, '    [] wlid%i=%i&currN=%i&contAction%i=%i&sink=0&timesteps<maxTime&tankFlag=%i -> (sink''=1);\n', j, currwlid, 2, j, currK, j);
                    elseif(nextPointwlid>=wlidMax || currwlid>=wlidMax)
                        fprintf(fid, '    [] wlid%i=%i&currN=%i&contAction%i=%i&sink=0&timesteps<maxTime&tankFlag=%i -> (sink''=1);\n', j, currwlid, 2, j, currK, j);
                    else
                        if(j==numTanks)
                            fprintf(fid, '    [] wlid%i=%i&currN=%i&contAction%i=%i&sink=0&timesteps<maxTime&tankFlag=%i -> (wlid%i''=%i)&(currN''=0)&(timesteps''=timesteps+1)&(tankFlag''=%i);\n', j, currwlid, 2, j, currK, j, j, round(nextPointwlid), 1);
                        else
                            fprintf(fid, '    [] wlid%i=%i&currN=%i&contAction%i=%i&sink=0&timesteps<maxTime&tankFlag=%i -> (wlid%i''=%i)&(currN''=2)&(tankFlag''=%i);\n', j, currwlid, 2, j, currK, j, j, round(nextPointwlid), j+1);
                        end
                    end
                end
            end
