...
def run(args):
    ...
    for exp in range(no_exps):
        prism_in = 'DimRem_automated_exp_results/PRISMInput{}.csv'.format(exp+1)
        prism_out = 'DimRem_automated_exp_results/PRISMLECModel{}.prism'.format(exp+1)
        ...
        prop = prop[:-1]
        prop += ')]'
        with open('DimRem_automated_exp_results/prop{}.props'.format(exp+1), 'w') as prop_file:
